...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if(user){
    const loginURL = `https://main--storied-halva-d5c778.netlify.app/admin`;
    await sendEmail({
      email: email,
...
const getUserProfile = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    res.status(404);
    throw new Error('Không tồn tại tài khoản này');
  }
});
...
const updateUser = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    res.json({
      message: `Cập nhật thông tin người dùng ${updatedUser._id} thành công`
    });
  } else {
    res.status(404);
